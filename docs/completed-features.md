...
- [x] 沉浸模式只看舞台：沉浸模式新增“只看舞台/显示界面”隐藏功能，可一键隐藏标题、歌词、播放列表、韵律面板、底部控制栏和空状态，只保留音乐视觉舞台；右上角恢复按钮在鼠标移入或键盘聚焦时显示，鼠标移开后自动隐藏；默认主题与 `animal-island` 主题均已检查，需求计划见 `docs/music-immersive-stage-only-mode-requirements.md`。
