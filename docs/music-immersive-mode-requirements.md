...
- [x] 参考 Mineradio 的播放启动阶段拆分方式，沉浸模式优先启动当前 `<audio>` 播放，歌词和辅助视觉准备延迟执行并可在切歌时取消。
- [x] 在线音源在沉浸模式中不再额外读取整首音频做离线节奏图，避免与当前播放流抢带宽或触发代理重复读取；在线歌曲使用实时频谱和安全视觉回退。
- [x] 网易云和酷狗在线歌曲增加运行态播放地址预取缓存，只用于当前进程内切歌加速，不写入本地音乐库或源码。
