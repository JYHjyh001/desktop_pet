...
16. AI 接口设置支持配置 OpenAI 兼容、DeepSeek、Anthropic、Gemini、Ollama 和自定义服务，并支持连接测试。
17. 宠物对话支持本地记忆系统，会保存最近聊天记录并把重要偏好、项目和事件提取为长期记忆，并可在设置中导入、导出或打开记忆文件目录。
...
6. 在“AI 接口”中配置服务商、Base URL、模型、API Key、系统提示词、温度和最大输出 Token，并点击“测试连接”确认接口可用；这些配置会保存到本机 `config.json`，用于宠物聊天对话。
