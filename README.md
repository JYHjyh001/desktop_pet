...
11. 宠物位置、自定义宠物形象、分类、快捷搜索、标签显示模式、AI 接口、窗口置顶、开机自启和自动常用设置保存到应用数据目录的 `config.json`。
12. 打开快捷入口时，前端只传 `app_id`，Rust 后端读取 `apps.json` 后按类型启动软件、打开文件夹或打开网站。
13. 系统托盘提供显示宠物、隐藏宠物、打开抽屉和退出程序。
14. 抽屉右上角可以直接切换缩略显示和详细显示。
15. 抽屉设置按入口管理、系统、AI 接口、记忆、窗口、更新和诊断分类组织。
16. AI 接口设置支持配置 OpenAI 兼容、DeepSeek、Anthropic、Gemini、Ollama 和自定义服务，支持连接测试，并可将多组连接保存为可切换的本机配置标签。
17. 宠物对话支持本地记忆系统，会保存最近聊天记录，并把称呼、偏好、边界、相处方式、习惯、目标、重要经历等陪伴型信息提取为长期记忆，并可在设置中查看、添加、编辑、删除、导入、导出或打开记忆文件目录。
...
6. 在“AI 接口”中配置服务商、Base URL、模型、API Key、系统提示词、温度和最大输出 Token，并点击“测试连接”确认接口可用；还可以把当前服务商、Base URL、模型和 API Key 保存为配置标签，点击标签切换活动连接，最多保留 20 个标签。活动连接与标签仅保存到本机 `config.json`，用于宠物聊天对话。
7. 在“记忆”中开启或关闭宠物记忆，查看、导入、导出、打开目录、删除或清空长期记忆。
8. 在“窗口”中分别控制宠物窗口和抽屉窗口是否始终置顶。
9. 在“更新”中查看当前版本，并检查发布者是否配置了新版下载入口。
10. 在“诊断”中查看当前 exe 路径和本机数据目录。
...
6. 如果启用了宠物记忆，对话时会自动保存最近聊天记录，并尝试从自然对话中把值得长期记住的称呼、偏好、边界、习惯、目标、相处方式、共同经历和生活变化保存到本机 `pet-memory.db`。
